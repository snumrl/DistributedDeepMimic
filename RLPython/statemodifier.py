...
class ClassicModifier():
    ...
    def apply_list(self, state):
        for i in range(state.shape[0]): state[i] = self.apply(state[i])
        return state
    
    def modify_list(self, state):
        res = state.clone()
        for i in range(state.shape[0]): res[i] = self.modify(state[i])
        return res
