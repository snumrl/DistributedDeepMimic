...
BuildFromFile(const std::string& filename){
	...
	for(XMLElement *body = skeldoc->FirstChildElement("Joint"); body != nullptr; body = body->NextSiblingElement("Joint")){
		...
		// body position
		XMLElement *bodyPosElem = body->FirstChildElement("BodyPosition");
		...
		// joint position
		XMLElement *jointPosElem = body->FirstChildElement("JointPosition");
		...
		// capsule
		XMLElement *shapeElem = body->FirstChildElement("Capsule");
...
//        }
			...
			// joint limit
			XMLElement *jointLimitElem = body->FirstChildElement("Limit");
			bool isLimitEnforced = false;
