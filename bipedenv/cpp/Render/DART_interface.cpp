	...
	const dart::dynamics::SkeletonPtr& skel, const Eigen::Vector3d& uniform_color, int type)
{
	for(int i=0;i<skel->getNumBodyNodes();i++)
	{
		...
		std::string name = bn->getName();
int idx = type >= shapeNodes.size()? shapeNodes.size()-1 : type;
		Eigen::Vector4d color = shapeNodes[idx]->getVisualAspect()->getRGBA();
		color.head<3>() *= 0.5;
		if(name == "FemurL" || name == "FemurR" || name == "TibiaL" || name == "TibiaR"){
			glPushMatrix();
			glColor4f(color[0], color[1], color[2], color[3]);
		...
		auto T = shapeNodes[idx]->getTransform();
		Eigen::Vector4d RGBA = Eigen::Vector4d::Ones();
		RGBA.segment<3>(0) = uniform_color;
		// RGBA = shapeNodes[idx]->getVisualAspect()->getRGBA();
		DrawShape(T,shapeNodes[idx]->getShape().get(), RGBA, uniform_color, name);
	}
}
