...
#include "Helper/CharacterConfigurations.h"
#include "Parameter.h"
#include <fstream>
#include <string>
#include <sstream>
...
void printdebug(Eigen::VectorXd x){
	for(int i = 0; i < x.size(); i++) std::cout << x[i] << " ";
	std::cout << std::endl;
}

Eigen::AngleAxisd DARTPositionToAngleAxisd(Eigen::Vector3d in){
	if( in.norm() < 1e-8 ) return Eigen::AngleAxisd::Identity();
	return Eigen::AngleAxisd(in.norm(), in.normalized());
}

Eigen::Quaterniond DARTPositionToQuaternion(Eigen::Vector3d in){
	return Eigen::Quaterniond(DARTPositionToAngleAxisd(in));
}
...
double getYrot(Eigen::Vector3d from, Eigen::Vector3d to){
	return getYrot(DARTPositionToQuaternion(from), DARTPositionToQuaternion(to));
}

double getYrot(Eigen::Vector3d to){
	return getYrot(Eigen::Quaterniond::Identity(), DARTPositionToQuaternion(to));
}
...
void positionRotateY(Eigen::VectorXd &pos, double dY){
	pos.segment(0, 3) = QuaternionToDARTPosition(Eigen::AngleAxisd(dY, Eigen::Vector3d::UnitY()) * DARTPositionToQuaternion(pos.segment(0, 3)));
	pos.segment(3, 3) = Eigen::Vector3d(pos[3] * cos(dY) + pos[5] * sin(dY), pos[4], pos[3] * -sin(dY) + pos[5] * cos(dY));
}

void positionTranslate(Eigen::VectorXd &pos, Eigen::Vector3d dx){
	pos.segment(3, 3) += dx;
}
